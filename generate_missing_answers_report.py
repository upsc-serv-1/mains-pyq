...
def main():
    ...
    run = title.add_run("UPSC Mains GS1: Coaching Institutes Solved Answers Audit")
    run.font.name = "Arial"
    run.font.size = Pt(18)
    run.bold = True
    ...
    p_desc.add_run("This report identifies the presence or absence of solved answers from 5 coaching institutes (Civilsdaily, Drishti IAS, PWOnlyIAS, Superkalam, and Unacademy) for all 275 GS1 syllabus questions spanning History, Society, and Geography.")
    ...
    # Set widths: Subject (0.8 in), Year (0.5 in), Question (2.7 in), 5 institutes (0.7 in each)
    col_widths = [Inches(0.8), Inches(0.5), Inches(2.7), Inches(0.7), Inches(0.7), Inches(0.7), Inches(0.7), Inches(0.7)]
    ...
    for res in audit_results:
        ...
        # Question text (strip Qxx. prefix to make it look clean)
        clean_q = re.sub(r'^Q\d+\.\s*', '', res["statement"]).strip()
        # Truncate to keep the file size and length manageable in Word, but keep enough for context
        if len(clean_q) > 130:
            clean_q = clean_q[:127] + "..."
    ...
    # Save document
    output_path = os.path.join(solved_dir, "gs1_missing_answers_report.docx")
    try:
        doc.save(output_path)
        print(f"Successfully generated and saved audit report to: {output_path}")
    except PermissionError:
        output_path_alt = os.path.join(solved_dir, "gs1_missing_answers_report_updated.docx")
